...
counts <- fread("/scratch/mjpete11/mitochondrial_paralogs/linear_models/data/data/batch_voom_qnorm_matrix.csv", sep=",") # float
reaction_activity_scores <- fread("/scratch/mjpete11/mitochondrial_paralogs/linear_models/data/data/Galaxy128_GTEx_liver_RAS_counts_vs_GTEx_heart_RAS_counts.tsv", sep="\t") # float
reaction_activity_scores
dim(reaction_activity_scores) # 263 3 
...
Yimp <- lm(counts.x ~ counts.y + SMRIN + SMTSISCH, data=combined_data, weights=NULL) 
