...
setwd("/scratch/mjpete11/mitochondrial_paralogs/linear_models/batch2_quantification/")
...
# Output
GENE_FILE = "/scratch/mjpete11/mitochondrial_paralogs/linear_models/data/data/simulated_batch2_error2_count_matrix_1000_samples.tsv"
...
# Run this if you add zero in the sample names in values < 9
#for (i in 1:1000){
#		if (i <= 9) {
...
files <- file.path("/scratch/mjpete11/mitochondrial_paralogs/linear_models/batch2_quantification/quantify_error2", strings, "quant.sf")
...
TxDb <- makeTxDbFromGFF(file = "/scratch/mjpete11/mitochondrial_paralogs/linear_models/batch2_quantification/index/gencode.v43.annotation.gff3", format=c("auto"))
k <- keys(TxDb, keytype = "TXNAME")                                             
tx2gene <- AnnotationDbi::select(TxDb, k, "GENEID", "TXNAME") # write output with gene ID/ trans ID (if one exists)                                 
head(tx2gene)                                                                   
...
dim(Counts_Gene) # 62269 1000
df_Gene <- data.frame(Counts_Gene, check.names = TRUE)
dim(df_Gene) # 62269 1000
df_Gene[1:5,1:5]
...
#df_Gene1[1:5,1:5]
#df_Gene1 <- df_Gene1 %>% relocate(ensembl_ID) %>% head()
#dim(df_Gene1) # 
...
#rm(dat)
#dat <- fread("/scratch/mjpete11/mitochondrial_paralogs/linear_models/data/simulated_batch2_error2_count_matrix_1000_samples.tsv", sep="\t")
#dat[1:5,1:5]
