...
new_data <- quantile_normalisation(heart2)
new_data

new_data2 <- quantile_normalisation(liver2)

#_______________________________________________________________________________ 
# Density plots
#_______________________________________________________________________________ 
# Density plot of unquantile normalized heart
edata = log2(new_data + 1)
edata = edata[rowMeans(edata) > 3, ]
colramp = colorRampPalette(c(3,"white",2))(20)
pdf("/scratch/mjpete11/linear_models/results2/heart_quantile_adjusted_density_plot.pdf")
...
colramp = colorRampPalette(c(3,"white",2))(20)
pdf("/scratch/mjpete11/linear_models/results2/heart_not_quantile_adjusted_density_plot.pdf")
plot(density(adata[,1]),col=colramp[1],lwd=3,ylim=c(0,.30))
for(i in 2:20){lines(density(adata[,i]),lwd=3,col=colramp[i])}
dev.off()

# Density plot of unquantile normalized heart
bdata = log2(new_data2 + 1)
bdata = bdata[rowMeans(bdata) > 3, ]
colramp = colorRampPalette(c(3,"white",2))(20)
pdf("/scratch/mjpete11/linear_models/results2/liver_quantile_adjusted_density_plot.pdf")
plot(density(bdata[,1]),col=colramp[1],lwd=3,ylim=c(0,.30))
for(i in 2:20){lines(density(bdata[,i]),lwd=3,col=colramp[i])}
dev.off()

# Density plot of quantile normalized heart
cdata = log2(liver2 + 1)
cdata = cdata[rowMeans(cdata) > 3, ]
cdata = as.matrix(cdata)
colramp = colorRampPalette(c(3,"white",2))(20)
pdf("/scratch/mjpete11/linear_models/results2/liver_not_quantile_adjusted_density_plot.pdf")
plot(density(cdata[,1]),col=colramp[1],lwd=3,ylim=c(0,.30))
for(i in 2:20){lines(density(cdata[,i]),lwd=3,col=colramp[i])}
dev.off()

#_______________________________________________________________________________ 
# Combine heart and liver dataframes and add RIN, ischemic time, and batch effects
#_______________________________________________________________________________ 
...
#_______________________________________________________________________________ 
batch <- organs$SMGEBTCH

modcombat <- model.matrix(~0 + organ, data=organs)

# Reshape organs dataset to create count matrix
organ_counts <- organs
organ_counts$organ <- NULL
organ_counts$SAMPID <- NULL
organ_counts$gene <- NULL
organ_counts <- organ_counts[-c(4:7)]
organ_counts <- as.matrix(organ_counts)

# Subset batch and count matrix because getting error that subscript is too long...
batch <- as.factor(batch[1:10])
test <- as.factor(organ_counts[1:10,])

# Batch correction
combat_edata <- ComBat(dat=test, batch=batch, mod=modcombat)

#_______________________________________________________________________________ 
# Combine heart and liver dataframes 
#_______________________________________________________________________________ 
