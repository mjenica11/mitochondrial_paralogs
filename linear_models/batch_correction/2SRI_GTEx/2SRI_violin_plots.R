...
any(base::duplicated(counts$"Description")) # TRUE
length(counts$"Description") # 51259 genes
length(which(duplicated(counts$"Description"))) # 882 are duplicate entries???
counts_no_duplicates <- counts[!duplicated(counts[,c("Description")]),]
nrow(counts_no_duplicates) # 50377
length(which(duplicated(counts_no_duplicates$"Description"))) # 0 duplicated gene names 
# Write filtered counts (genes with >5 counts in at least 1 sample) without duplicate rows to file
#write.table(counts_no_duplicates, "/scratch/mjpete11/linear_models/data/filtered_5_counts_no_duplicates.csv", sep=",")
counts_no_duplicates <- read.table("/scratch/mjpete11/linear_models/data/filtered_5_counts_no_duplicates.csv", sep=",")

# Subset the SLC25 genes
sub_df <- counts_no_duplicates[counts_no_duplicates$"Description" %in% SLC, ]
...
# Change the name of the 'variable' column to 'SAMPID' 
...
# Dataframe with only samples from people who donated both a heart and liver: plotting_df
# e.g. paired samples
plotting_df_tmp <- merge(heart4, liver4, by="SUBJID") 
length(unique(plotting_df_tmp$SUBJID)) # 148 individuals
tmp1 <- plotting_df_tmp[,c("SUBJID", "gene.x", "organ.x", "SAMPID.x", "value.x", "SMRIN.x", "SMTSISCH.x", "SMGEBTCH.x", "SMGEBTCHT.x")]
tmp2 <- plotting_df_tmp[,c("SUBJID", "gene.y", "organ.y", "SAMPID.y", "value.y", "SMRIN.y", "SMTSISCH.y", "SMGEBTCH.x", "SMGEBTCHT.x")]
...
plotting_df0 <- rbind(tmp1, tmp2)

# Are any rows duplicated?
any(duplicated(plotting_df0)) # TRUE

# Drop duplicated rows
plotting_df <- plotting_df0[!duplicated(plotting_df0),]
length(unique(plotting_df_tmp$SUBJID)) # 148 individuals

# Change the name of 'value' column to 'combat_seq_counts'
colnames(plotting_df)[5] <- "combat_seq_counts"

# Convert combat_seq adjusted counts to log2 and CPM
# Values of zero will be set to 2
plotting_df$log2_cpm <- cpm(plotting_df$combat_seq_counts, log=TRUE, prior.count=0.5)

# Drop samples from the counts df that are not from paired heart or liver
#keeps <- as.vector(plotting_df$SAMPID)
#counts2 <- subset(counts, select=keeps) 

# Are there the same number of samples in the counts df and the plotting_df metadata df? 
#ncol(counts2)==nrow(plotting_df) # TRUE

# Set y lim
range(plotting_df$combat_seq_counts) # c(0 229728) 
range(plotting_df$log2_cpm) # c(-6.72, 12.09) 

# Regress out effect of RIN and ischemic time and remove from the model 
# before making the violin plots

# Create a list of dataframes with the 2SRI adjusted value so I can see
# what the mean/median/sd is outside the plotting function
contained <- list()
for (paralog in SLC){
	datt <- plotting_df %>% filter(gene == paralog)
	datt2 <- na.omit(datt)
...
median(plotting_df1$fitted_values) # 4.01
mean(plotting_df1$fitted_values) # 3.32
sd(plotting_df1$fitted_values) # 3.66
sd(plotting_df1$fitted_values) * 6 # +/- 21.97 
above <- 3.29 + 21.12 # 24.41 
below <- 3.29 - 21.12 # -17.83
above;below

# Are there any samples outside of this range?
range(plotting_df1$fitted_values) # -7.79, 11.9 --> No 
...
gaussian_2SRI_violin_plots <- function(GENE){
    ...
    # Regress out effect of RIN and ischemic time on SLC
    model1 <- glm(log2_cpm ~ SMRIN + SMTSISCH, data=dat)
	# Add fitted values to dataframe
	dat$resids <- residuals(model1)
	# Regress organ on SLC using previous fitted regression values as offset
	model2 <- glm(log2_cpm ~ organ + resids, data=dat)
...
#	corrected_pval <- p.adjust(p_val, method="bonferroni", n=53)
    # Violin plot
	p <- ggplot(dat, aes(x = organ, y = fitted_values, fill=organ)) +
				stat_compare_means(method = "wilcox.test", 
								   aes(label = paste("adj.p_value =", after_stat(!!str2lang("p.adj"))*53)), 
								   label.x = 1.25, 
								   label.y = max(dat[["fitted_values"]]) + 1.5,
								   paired = TRUE) +
				...
				ggsave(paste0("/scratch/mjpete11/linear_models/batch_correction/2SRI_GTEx/gaussian_2SRI_violin_plots/", GENE, ".png"), p, device="png")
}
...
#	dat <- plotting_df2 %>% filter(gene == GENE)
#    dat2 <- na.omit(dat)
