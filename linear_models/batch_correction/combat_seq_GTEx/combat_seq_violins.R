...
#plotting_df <- read.csv("/scratch/mjpete11/linear_models/data/plotting_df.csv", sep=",")

# Remove samples >6 standard deviations away
median(plotting_df$log2_cpm) # 4.00
mean(plotting_df$log2_cpm) # 3.32
sd(plotting_df$log2_cpm) # 3.67
sd(plotting_df$log2_cpm) * 6 # +/- 22.1 
above <- 3.43 + 20.6 # 24.03
below <- 3.43 - 20.6 # -17.17
above;below

# Are there any samples outside of this range?
range(plotting_df$log2_cpm) # -6.72 to 12.1 
...
violin <- function(GENE){
	     ...
	     p <- ggplot(dat, aes(x = organ, y = log2_cpm, fill = organ)) +
	   		  ...
	   		  ggsave(paste0("/scratch/mjpete11/linear_models/batch_correction/combat_seq_GTEx/plots/", GENE, ".png"), device="png")
}
