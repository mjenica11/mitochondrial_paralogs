...
# Add sample ID col to dataframe
...
any(duplicated(plotting_df_tmp)) # FALSE
...
# Double check merging the dataframes resulted as expected
nrow(plotting_df)==nrow(tmp1_2)+nrow(tmp2_2) # TRUE
any(duplicated(plotting_df)) # FALSE

# Expected number of rows?
148 * 53 * 2 # 15,688
nrow(plotting_df)==15688 # TRUE
any(duplicated(plotting_df)) # FALSE

# Convert the counts to log2(CPM) scale and save in separate column`
plotting_df$log2_cpm <- cpm(plotting_df$value, log=TRUE, prior.count=0.5)

# Write plotting_df df to file (02-20-2022)
#write.table(plotting_df, "/scratch/mjpete11/linear_models/data/plotting_df.csv", sep=",")

# Remove samples >6 standard deviations away
median(plotting_df$log2_cpm) # 3.87
mean(plotting_df$log2_cpm) # 3.17
sd(plotting_df$log2_cpm) # 3.71
sd(plotting_df$log2_cpm) * 6 # +/- 22.3 
...
violin <- function(GENE){
	     ...
	     p <- ggplot(dat, aes(x = organ, y = log2_cpm, fill = organ)) +
			  ...
			  stat_compare_means(method = "wilcox.test", 
	   		  ...
	   		  ggsave(paste0("/scratch/mjpete11/linear_models/batch_correction/no_batch_correction_GTEx/no_batch_violin_plots/", GENE, ".png"), device="png")
}
