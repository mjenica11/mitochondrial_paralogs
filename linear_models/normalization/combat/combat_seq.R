...
#counts3 <- write.csv("/scratch/mjpete11/linear_models/data/filtered_counts.csv")
...
setdiff(SLC, counts2$'Description')  # No
...
combat_edata <- ComBat_seq(counts=as.matrix(counts2[,-c(1)]), batch=batch)

# Append gene names back 
combat <- as.data.frame(combat_edata)
combat$"Description" <- counts2$"Description"
