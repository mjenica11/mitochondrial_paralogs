...
# Make a table of p-values from ComBat normalized counts
library(tidyr)
library(dplyr)
library(purrr)

organs$counts <- organs$value 
organs$counts <- as.numeric(organs$counts)
...
res <- A1 %>% group_by(organ) %>% wilcox.test(counts~organ, data=.)
res$p.value

	


# Function to make list of p-vals per gene 
wilcox.test(counts ~ organ, data = A1)$p.value

p_vals <- function(GENE){
	dat <- organs %>% filter(gene == GENE) %>% na.omit(dat)
#	dat$organ <- factor(dat$organ, levels=c("heart","liver"))
	res <- dat %>% group_by(organ) %>% wilcox.test(counts~organ, data = .)
	return(res$p.value)
}
SLCs <- SLCs[SLCs != "SLC25A7"]
res2 <- Map(p_vals, GENE=SLCs)
res2

# Make a table
write.csv(stack(res2), "/scratch/mjpete11/linear_models/u_test/combat_pvals.csv")
...
p + stat_compare_means(aes(label=..p.adj..))
dev.off()
